<template>
   <div class="min-h-screen bg-gray-200 flex flex-col md:flex-row items-center justify-center py-12">
      <div class="p-4 max-w-md w-full bg-white shadow-md rounded-md">
         <div class="text-center mb-4">
            <img :src="picture" alt="User Picture" class="w-24 h-24 rounded-full mx-auto">
         ...
         <button v-if="isAddFriendVisible == false" @click="showAddFriend" class="w-full bg-blue-500 text-white py-2 px-4 rounded-md hover:bg-blue-600">添加好友</button>
         
         <div v-if="isAddFriendVisible" id="addFriend" class="md:ml-4 mt-4 md:mt-0 flex-grow">
           <div class="mt-4 flex items-center">
              <input type="text" placeholder="輸入要加入的fastChatId" v-model="searchUserKey" class="flex-grow p-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
              <button @click="searchFriend" class="ml-2 bg-green-500 text-white py-2 px-4 rounded-md hover:bg-green-600">搜尋</button>
           </div>
              ...
              <div v-if="addFriendState == 'canJoin'">
                 <img v-if="searchUser.picture" :src="searchUser.picture" alt="Search User Picture" class="w-16 h-16 rounded-full mx-auto mb-2">
                   ...
                   <div  class="mt-6 flex justify-between">
                      <button class=" mt-2 bg-gray-500 text-white py-2 px-4 rounded-md hover:bg-gray-600">取消</button>
           ...
           <div v-else-if="searchUser.fastChatId == null && isShowHiddenUser" class="mt-4 text-center ">
            <svg class="w-16 h-16 rounded-full mx-auto mb-2" width="2391" height="2391" viewBox="0 0 2391 2391" fill="none" xmlns="http://www.w3.org/2000/svg">
                     ...
                     <rect x="1089.36" y="1489.45" width="307" height="56.7835" transform="rotate(-13.7196 1089.36 1489.45)" fill="#828282"/>
                     <rect x="1235.71" y="1022.19" width="54" height="447.959" transform="rotate(-13.7196 1235.71 1022.19)" fill="#828282"/>
         ...
         <div class="mt-6 flex justify-between">
            <button class="bg-gray-500 text-white py-2 px-4 rounded-md hover:bg-gray-600">取消</button>
            <button class="bg-blue-500 text-white py-2 px-4 rounded-md hover:bg-blue-600">保存</button>
         </div>
      </div>
   </div>
</template>
...
let addFriend = () => {
  userStore.addFriend(searchUser.value.sub);
};
</script>
