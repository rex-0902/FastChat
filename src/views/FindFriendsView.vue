<template>
  <div
    id="FindFriends"
    class="z-0 overflow-auto h-[calc(100%-130px)] w-[420px]"
  >
      ...
      >
        ...
        <div class="w-full">
          <div class="flex justify-between items-center">
            <div class="text-[15px] text-slate-300">
              {{ user.lastName }}{{ user.firstName }}
            </div>
          </div>
          <div class="flex items-center">
            <div class="text-[15px] text-slate-400">
              Hi, 我是{{ user.lastName }}{{ user.firstName }} !
            </div>
          </div>
        </div>
      </div>
...
const checkIsThereAConversationId = (sub1, sub2) => {
  ...
  result = result.filter((item) => item.sub1 === sub1 && item.sub2 === sub2);
...
const createNewChat = async (user) => {
  ...
  if (checkIsThereAConversationId(sub.value, user.sub) !== 0) {
    ...
    try {
      await userStore.getChatById(
        checkIsThereAConversationId(sub.value, user.sub).id
      );
      let data = {
        id: checkIsThereAConversationId(sub.value, user.sub).id,
        key1: "sub1HasViewed",
      ...
      if (checkIsThereAConversationId(sub.value, user.sub).sub1 === sub.value) {
        ...
        checkIsThereAConversationId(sub.value, user.sub).sub2 === sub.value
      ) {
        data.val1 = true;
        data.val2 = true;
      }
